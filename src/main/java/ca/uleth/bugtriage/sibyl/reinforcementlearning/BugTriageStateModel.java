...
public class BugTriageStateModel implements FullStateModel {
	...
	public State sample(State state, Action action) {
		...
		BugReportTerm brt = (BugReportTerm) state;
		Instance nextReport;
			...
			if (reportItr.hasNext()) {
				...
				for (int i = 0; i < nextReport.numAttributes(); i++) {
					...
					if (nextReport.value(i) > 0) {
						...
						BugReportTerm termState;
						if (termItr == null) {
							termState = new BugReportTerm(term, nextReport);
						} else {
							termState = (BugReportTerm) brt.copyWithName(term);
				...
				throw new RuntimeException("No more reports to sample!");
			}
		}
		return termItr.next();
	}
