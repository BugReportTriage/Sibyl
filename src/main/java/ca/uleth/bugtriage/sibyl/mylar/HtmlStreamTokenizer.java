...
package ca.uleth.bugtriage.sibyl.mylar;
...
public class HtmlStreamTokenizer {
	...
	/** Allow class client to choose if tag atributes are escaped or not */
		...
		do {
			...
			if (state == State.TEXT) {
				...
				if (ch == '<' && textBuffer.length() == 0) {
					textBuffer.append("<<");
					state = State.TEXT;
				} else if (ch == '-' && textBuffer.length() == 2
						&& textBuffer.charAt(1) == '-'
						&& textBuffer.charAt(0) == '!') {
					textBuffer.setLength(0);
					state = State.COMMENT;
	...
	private static void parseAttributes(HtmlTag tag, String s, int i, boolean escapeValues) throws ParseException {
		while (i < s.length()) {
			...
			int start = i;
			for (; i < s.length() && !Character.isWhitespace(s.charAt(i))
					&& s.charAt(i) != '='; i++) {
				// just move forward
			}
	...
	public static class Token {
		 ...
		 */
		protected Token(StringBuffer text, StringBuffer whitespace,
				boolean comment) {
			if (comment) {
				type = COMMENT;
			} else {
