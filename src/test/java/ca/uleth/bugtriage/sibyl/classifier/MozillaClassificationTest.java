...
import ca.uleth.bugtriage.sibyl.AllTests;
...
public class MozillaClassificationTest {
	...
	public void testFixedHeuristic() {
		Dataset dataset = AllTests.getFirefox_2Bugs();
		HeuristicClassifier hClassifier = Project.FIREFOX.heuristic.getClassifier();
		List<BugReport> reports = new ArrayList<BugReport>(dataset.getReports());
	...
	public void testBuildClassisifer() {
		Dataset dataset = AllTests.getFirefox_1Month();
		ClassifierType classifierType = ClassifierType.SVM;
		Heuristic heuristic = Heuristic.MOZILLA;

		assertEquals(dataset.getReports().size(),
				dataset.getTrainingReports().size() + dataset.getTestingReports().size());
	...
	public void testUseClassifier() throws FileNotFoundException {
		...
		/*
		 * // Loading not working Dataset dataset = new
		 * BugzillaDataset(Project.FIREFOX); File classifierFile = new
		 * File(Environment.getClassifierDir() + dataset.getProject().name +
		 * ".arff"); TriageClassifier classifier =
		 * MLClassifier.load(classifierFile);
		 */
